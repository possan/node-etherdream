...
EtherDream.connect(IP, PORT, function(conn) {
	...
	function drawDigit(framedata,n,x,y,w,h,r,g,b) {
		...
		//  4444
		var segs = segdata[n];
		if (segs[0] == 1) addline(framedata, x-w,y-h, x+w,y-h, r,g,b);
		if (segs[1] == 1) addline(framedata, x+w,y-h, x+w,y, r,g,b);
		if (segs[2] == 1) addline(framedata, x+w,y, x-w,y,	r,g,b);
		if (segs[3] == 1) addline(framedata, x-w,y, x-w,y+h, r,g,b);
		if (segs[4] == 1) addline(framedata, x-w,y+h, x+w,y+h, r,g,b);
		if (segs[5] == 1) addline(framedata, x+w,y+h, x+w,y, r,g,b);
		if (segs[6] == 1) addline(framedata, x-w,y, x-w,y-h, r,g,b);
	}
	...
	function nextframe(phase) {
		...
		*/
		...
		if (shouldfill) {
			...
			// glMatrix.mat4.rotate(t0, model, phase / 420.0, [1,0,0]);
			// glMatrix.mat4.rotate(t1, t0, phase / 20.0, [0,1,0]);
			// glMatrix.mat4.rotate(model, t1, phase / 250.0, [0,0,1]);
			// glMatrix.mat4.translate(model, model, [0, 0, -300000]);
			// glMatrix.mat4.scale(model, model, [0.3, 0.3, 0.3]);

			glMatrix.mat4.identity(view);
			// glMatrix.mat4.lookAt(view, [3 * Math.sin(phase / 8.0), 3 * Math.cos(phase / 9.0), 5], [0,0,0], [0,-1,0]);
			...
			drawDigit(linedata, n2, -15, 0, 5, 10, 65535,0,0);
			drawDigit(linedata, n1, 0, 0, 5, 10, 65535,0,0);
			drawDigit(linedata, n0, 15, 0, 5, 10, 65535,0,0);
			...
			for(var i=0; i<linedata.length; i++) {
				var l = linedata[i];
				var v0 = glMatrix.vec4.fromValues(l.x0, l.y0, 0.2 * Math.sin(l.x0 / 24.0), 1.0);
				var v1 = glMatrix.vec4.fromValues(l.x1, l.y1, 0.2 * Math.sin(l.x1 / 24.0), 1.0);

				glMatrix.vec4.transformMat4(v0, v0, finalmat);
				glMatrix.vec4.transformMat4(v1, v1, finalmat);

				drawline(framedata,
					v0[0] * 1500.0 / (1.0 + v0[2] / 30.0),
					v0[1] * 1500.0 / (1.0 + v0[2] / 30.0),
					v1[0] * 1500.0 / (1.0 + v1[2] / 30.0),
					v1[1] * 1500.0 / (1.0 + v1[2] / 30.0),
					l.r, l.g, l.b);
			}
